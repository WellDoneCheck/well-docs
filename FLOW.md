...
**步驟取捨說明:** 上述六步為對外/對評審呈現用的精簡版本,原內部細節共八個動作點,其中「子圖存入 Object Storage」與「manifest 回報」已內含於步驟 3,不另編號;「presigned URL 繞過主後端」(步驟1)與「結果聚合寫入」(步驟5)為系統關鍵技術決策,不可省略,省略會使讀者誤判元件間的實際關係或資料寫入時機。
